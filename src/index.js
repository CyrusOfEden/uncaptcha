(function() {
  let settings = {
    url: "http://127.0.0.1:9292", // url of the captcha server. adjust as necessary.
    duration: 5000 // 5 seconds
  };
  // the base64 encodings of all graphics
  let images = {
  ...
  // Completely removes the DOM node and replaces it with a clone.
  ...
  setAttr(stylesheet, "rel", "stylesheet");
  setAttr(stylesheet, "type", "text/css");
  setAttr(stylesheet, "href", path.getCSS());
  // Add the stylesheet to the current document
  document.head.appendChild(stylesheet);

  const imageSrc = (type, image) => `data:image/${type};base64,${image}`;
  ...
  function bootstrap() {
    // Have these be lazy functions that return the elements
    // in case the Node is replaced.
  ...
  function play() {
    ...
    request("get", path.getCaptcha(), (res) => {
      ...
      // Call the input function after `settings.duration` milliseconds
  ...
  function input() {
    let container = elements.container();
    remClass(container, "play");
    addClass(container, "input");

    setActionText(state.text[1]);

    state.sequence = ""; // reset the stored sequence
    // Watch for clicks on the buttons
    watch(elements.buttons(), "click", (evt) => {
      // evt.target ends up being the element that was clicked
  ...
  function verify() {
    ...
    request("post", path.checkCaptcha(state.id, state.sequence), (res) => {
      // status is the state of the validation of the captcha.
